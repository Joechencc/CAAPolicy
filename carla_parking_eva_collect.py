...
from agent.parking_agent_collect import ParkingAgent, show_control_info


def game_loop(args, folder_name):
    pygame.init()
    pygame.font.init()
    ...
    try:
        ...
        parking_agent = ParkingAgent(network_evaluator, args, folder_name)
        ...
        while True:
            ...
            if controller.parse_events(client, network_evaluator.world, clock):
                return
            parking_agent.tick(folder_name)
...
def main():
    ...
    argparser.add_argument(
        '--output_path',
        default='/media/chao1804/2TB-DRIVE/Town_dynamic',
        help='path to save dynamic file')
    ...
    folder_name = os.path.join(args.output_path, datetime.now().strftime("%m_%d_%y_%H_%M"))
