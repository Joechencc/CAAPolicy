...
class CarlaDataset(torch.utils.data.Dataset):
            ...
            for task_path in tqdm(all_tasks, desc="Processing tasks"):
                ...
                for frame in range(self.cfg.hist_frame_nums, total_frames - self.cfg.future_frame_nums):
                    ...
                    if not os.path.isfile(filename):
                        ...
                        continue
                        frame_no = all_frameno[index:index+self.cfg.future_frame_nums].copy()
                        frame_no = self.ensure_incremental(frame_no, self.cfg.future_frame_nums)
    ...
    def __getitem__(self, index):
        ...
        #                                     target_slot=self.target_point[index])
        # elif self.cfg.feature_encoder == "conet":
        frame_no = self.all_frameno[index:index+self.cfg.future_frame_nums].copy()
        frame_no = self.ensure_incremental(frame_no, self.cfg.future_frame_nums)
...
class ProcessSemantic3D:
    ...
    def __call__(self, voxel, target_slot, traj=None):
        ...
        #####################################
        boundary_mask = np.zeros_like(interpolated_segmentation, dtype=bool)
        labels = np.unique(interpolated_segmentation)

        for i, label in enumerate(labels[:-1]):
            label_mask = (interpolated_segmentation == label)
            dilated_label = binary_dilation(label_mask)
