    ...
    def get_data(self):
        ...
        for task_path in all_tasks:
            ...
            for frame in range(self.cfg.hist_frame_nums, total_frames - self.cfg.future_frame_nums):
                ...
                # Load target parking goal
                with open(task_path + f"/parking_goal/0001.json", "r") as read_file:
                    data = json.load(read_file)
                parking_goal_world = [data['x'], data['y'], data['yaw']]

                # Convert parking goal to ego frame
                parking_goal_ego = convert_slot_coord(ego_trans, parking_goal_world)  

                # Store parking goal
                goal_idx = len(self.target_point)
                self.target_point.append(parking_goal_ego)

                # Initialize relative target storage for this goal
                self.relative_target_point[goal_idx] = []
        ...
        for goal_idx in self.relative_target_point:
            self.relative_target_point[goal_idx] = np.array(self.relative_target_point[goal_idx], dtype=np.float32)
