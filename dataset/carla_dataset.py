...
def tokenize_dynamics(speed, acc_x, acc_y, token_nums=204):
    """
    Tokenize dynamics signals
    :param speed: [0,max_speed] - speed in m/s
    :param acc_x: [-max_acc,max_acc] - x acceleration in m/s^2
    :param acc_y: [-max_acc,max_acc] - y acceleration in m/s^2
    :param token_nums: size of token
    :param max_speed: maximum speed value in m/s
    :param max_acc: maximum acceleration value in m/s^2
    :return: tokenized dynamics values [speed_token, acc_x_token, acc_y_token]
    """
    max_speed = 20
    max_acc = 12
    valid_token = token_nums - 4
    third_token = valid_token / 3
    
    # Speed token (always positive)
    speed_normalized = min(speed / max_speed, 1)
    speed_token = int(speed_normalized * third_token)
    
    # Acc_x token (can be positive or negative)
    acc_x_normalized = (max(min(acc_x / max_acc, 1), -1) + 1) / 2
    acc_x_token = int(acc_x_normalized * third_token)
    
    # Acc_y token (can be positive or negative)
    acc_y_normalized = (max(min(acc_y / max_acc, 1), -1) + 1) / 2
    acc_y_token = int(acc_y_normalized * third_token)
    
    return [speed_token, acc_x_token, acc_y_token]


def detokenize_dynamics(token_list, token_nums=204):
    """
    Detokenize dynamics signals
    :param token_list: [speed_token, acc_x_token, acc_y_token]
    :param token_nums: size of token number
    :param max_speed: maximum speed value in m/s
    :param max_acc: maximum acceleration value in m/s^2
    :return: dynamics signal values [speed, acc_x, acc_y]
    """
    max_speed = 20
    max_acc = 12
    valid_token = token_nums - 4
    third_token = float(valid_token / 3)
    
    speed = (token_list[0] / third_token) * max_speed
    acc_x = ((token_list[1] / third_token) * 2 - 1) * max_acc
    acc_y = ((token_list[2] / third_token) * 2 - 1) * max_acc
    
    return [speed, acc_x, acc_y]
...
class CarlaDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir, is_train, config):
        ...
        # Add raw dynamics lists for future frames
        self.future_velocity = []
        self.future_acc_x = []
        self.future_acc_y = []
    ...
    def get_data(self):
        ...
        for task_path in all_tasks:
            ...
            for frame in range(self.cfg.hist_frame_nums, total_frames - self.cfg.future_frame_nums):
                ...
                for i in range(10): # 10steps * 0.1s/step = 1 sec future dynamics
                    if frame + 1 + i < total_frames:
                        with open(task_path + f"/measurements/{str(frame + 1 + i).zfill(4)}.json", "r") as read_file:
                            data = json.load(read_file)
                        velocity = data['speed']
                        acc_x = data['acc_x']
                        ...
                        velocity, acc_x, acc_y = 0.0, 0.0, 0.0

                    # Process dynamics signals in the same way
                    dynamics.append(tokenize_dynamics(velocity, acc_x, acc_y, self.cfg.token_nums))
                    future_velocities.append(data['speed'])
                    future_acc_xs.append(data['acc_x'])
                    future_acc_ys.append(data['acc_y'])
                ...
                dynamics = [item for sublist in dynamics for item in sublist]
                dynamics.insert(0, self.BOS_token)
                dynamics.append(self.EOS_token)
                dynamics.append(self.PAD_token)
                self.dynamics_tokens.append(dynamics)

                self.future_velocity.append(future_velocities)
                self.future_acc_x.append(future_acc_xs)
                self.future_acc_y.append(future_acc_ys)
        ...
        self.dynamics_tokens = np.array(self.dynamics_tokens).astype(np.int64)
        self.future_velocity = np.array(self.future_velocity).astype(np.float32)
        self.future_acc_x = np.array(self.future_acc_x).astype(np.float32)
        self.future_acc_y = np.array(self.future_acc_y).astype(np.float32)
    ...
    def __getitem__(self, index):
        ...
        keys = ['image', 'depth', 'extrinsics', 'intrinsics', 'target_point', 'ego_motion', 'segmentation',
                ...
                'gt_dynamics', 'gt_future_velocity', 'gt_future_acc_x', 'gt_future_acc_y'] 
        
        # keys for future ego motion
        for i in range(self.cfg.future_frame_nums):
            keys.append(f'future_ego_motion_{i+1}')
        ...
        # Add future ego_motion data
        for i in range(self.cfg.future_frame_nums):
            future_index = index + i + 1
            if future_index < len(self.velocity):
                future_ego_motion = np.column_stack((
                    self.velocity[future_index], 
                    self.acc_x[future_index], 
                    self.acc_y[future_index]
                ))
                data[f'future_ego_motion_{i+1}'] = torch.from_numpy(future_ego_motion)
            else:
                # Handle edge case where we don't have enough future frames
                # Just duplicate the last available frame
                data[f'future_ego_motion_{i+1}'] = data['ego_motion'].clone()
        ...
        # Add gt dynamics tokens to the data
        data['gt_dynamics'] = torch.from_numpy(self.dynamics_tokens[index])
        
        # Add gt raw dynamics values to the data
        data['gt_future_velocity'] = torch.from_numpy(self.future_velocity[index])
        data['gt_future_acc_x'] = torch.from_numpy(self.future_acc_x[index])
        data['gt_future_acc_y'] = torch.from_numpy(self.future_acc_y[index])
