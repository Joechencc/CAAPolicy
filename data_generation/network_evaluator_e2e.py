...
from data_generation.world_e2e import World
import json
from threading import Thread

from utils.align_lidar_point_clouds import transform_and_merge_point_clouds
from utils.ply2voxel import voxelization_save, read_ply_with_properties,voxelization_from_np_save
...
class NetworkEvaluator:
    ...
    def save_sensor_data(self, parking_goal):
        ...
        # align severa point clouds and do voxelization and save to disk
        self.align_lidar_point_cloud()

        logging.info('saved sensor data for task %d in %s', self._task_index, str(cur_save_path))
    def align_lidar_point_cloud(self):
        cur_save_path = pathlib.Path(self._save_path) / ('task' + str(self._task_index))
        voxel_path = cur_save_path / "voxel"
        voxel_path.mkdir(parents=True, exist_ok=True)  # Ensure the voxel directory exists

        first_key = next(iter(self._lidar_config))
        lidar_data_dir = cur_save_path / first_key

        for ply_file in tqdm(lidar_data_dir.iterdir(), desc="Processing voxel"):
            if ply_file.suffix == '.ply':
                tmp_point_clouds = []
                tmp_specs = []

                # Iterate through all lidar configurations
                for key, value in self._lidar_config.items():
                    file_path = cur_save_path / key / ply_file.name
                    if file_path.exists():
                        tmp_point_clouds.append(read_ply_with_properties(str(file_path)))
                        tmp_specs.append([value["x"],value["y"],value["z"]])
