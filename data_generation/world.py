...
class World(object):
    ...
    def spawn_lidar_occ(self, lidar_specs):
        print('spwan occ lidar is called!!!!\n\n')
        ...
        lidar_location = carla.Location(x=0, y=0, z=1.6)  ### we may later update the z to 2.5
        lidar_rotation = carla.Rotation(pitch=0.0, roll=0.0, yaw=0.0)
        lidar_transform = carla.Transform(lidar_location, lidar_rotation)
        lidar = self.world.spawn_actor(lidar_bp, lidar_transform, attach_to=self.player,
                                       attachment_type=carla.AttachmentType.Rigid)
        lidar.listen(lambda data: sensor_callback(data, self._sensor_queue, "feng_lidar_occ"))
    ...
    def init_sensors(self):
        ...
        self.setup_sensors()
        lidar_specs_occ = {}
        lidar_specs_occ['rotation_frequency'] = 100
        lidar_specs_occ['points_per_second'] = 500000
        lidar_specs_occ['channels'] = 32
        lidar_specs_occ['upper_fov'] = 10
        lidar_specs_occ['lower_fov'] = -30
        lidar_specs_occ['range'] = 16  # 100 meters
        lidar_specs_occ['atmosphere_attenuation_rate'] = 0.1
        ##### the following setting is for noise ######### else lidar_specs['noise_stddev'] = 0.2
        lidar_specs_occ['dropoff_general_rate'] = 0.0
        lidar_specs_occ['dropoff_intensity_limit'] = 1.0
        lidar_specs_occ['dropoff_zero_intensity'] = 0.0

        self.spawn_lidar_occ(lidar_specs_occ)  ### the location of lidar is defined in the spawn_lidar method itself.
    ...
    def spawn_rgb_camera(self, sensor_id, sensor_spec):
        ...
        if "fov" in sensor_spec and sensor_spec["fov"] is not None:
            bp.set_attribute("fov", str(sensor_spec["fov"]))
        if "width" in sensor_spec and sensor_spec["width"] is not None:
            bp.set_attribute('image_size_x', str(sensor_spec["width"]))
        if "height" in sensor_spec and sensor_spec["height"] is not None:
