...
class DynamicsPredict(nn.Module):
    ...
    def decoder(self, encoder_out, tgt_embedding, tgt_mask, tgt_padding_mask):
        ...
        pred_dynamics = self.tf_decoder(tgt=tgt_embedding,
                                        memory=encoder_out,
                                        tgt_mask=tgt_mask,
                                        tgt_key_padding_mask=tgt_padding_mask)
        pred_dynamics = pred_dynamics.transpose(0, 1)
    ...
    def forward(self, encoder_out, tgt):
        ...
        pred_dynamics = self.decoder(encoder_out, tgt_embedding, tgt_mask, tgt_padding_mask)
        pred_dynamics = self.output(pred_dynamics)
        return pred_dynamics

    def predict(self, encoder_out, tgt):
        length = tgt.size(1)

        padding = torch.ones(tgt.size(0), self.tf_de_tgt_dim - length - 1).fill_(self.pad_idx).long().to('cuda')
        ...
        pred_dynamics = self.decoder(encoder_out, tgt_embedding, tgt_mask, tgt_padding_mask)
        pred_dynamics = self.output(pred_dynamics)[:, length - 1, :]

        pred_dynamics = torch.softmax(pred_dynamics, dim=-1)
        pred_dynamics = pred_dynamics.argmax(dim=-1).view(-1, 1)
        return pred_dynamics