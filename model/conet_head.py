...
class CONetHead(nn.Module):
    def __init__(
        ...
        self.occ_pred_conv = nn.Sequential(
                ...
                build_conv_layer(conv_cfg, in_channels=32, 
        ...
        if self.cascade_ratio != 1: 
            if self.sample_from_voxel or self.sample_from_img:
                fine_mlp_input_dim = 0 if not self.sample_from_voxel else 64
                if sample_from_img:
                    ...
                    fine_mlp_input_dim += 32

                self.fine_mlp = nn.Sequential(
                    nn.Linear(fine_mlp_input_dim, 32),
                    nn.GroupNorm(16, 32),
                    nn.ReLU(inplace=True),
