...
from model.dynamics_predict import DynamicsPredict
...
class ParkingModel(nn.Module):
    def __init__(self, cfg: Configuration):
        ...
        self.dynamics_predict = DynamicsPredict(self.cfg)
    ...
    def forward(self, data):
        ...
        # Predict dynamics
        pred_dynamics = self.dynamics_predict(fuse_feature, data['gt_dynamics'].cuda())
        
        return pred_control, pred_dynamics, pred_segmentation, pred_depth
    ...
    def predict(self, data):
        ...
        # Predict dynamics
        pred_multi_dynamics = data['gt_dynamics'].cuda()
        for i in range(3): 
            pred_dynamics = self.dynamics_predict.predict(fuse_feature, pred_multi_dynamics)
            pred_multi_dynamics = torch.cat([pred_multi_dynamics, pred_dynamics], dim=1)
        
        return pred_multi_controls, pred_multi_dynamics, pred_segmentation, pred_depth, bev_target