...
class ConetEncoder(nn.Module):
    def __init__(self, in_channel):
        ...
        self.layer3 = self._create_layer3d(BasicBlock3D, 128, 256, 2, stride=(2, 2, 4))
    ...
    def forward(self, x):
        ...
        x = self.layer1(x) # torch.Size([2, 64, 64, 64, 16])
        x = self.layer2(x) # torch.Size([2, 128, 32, 32, 8])
        x = self.layer3(x) # torch.Size([2, 256, 16, 16, 2])
        # x = self.layer4(x) #([1, 512, 8, 8, 2])

        x = torch.flatten(x, 2) # torch.Size([2, 256, 512])
        return x
