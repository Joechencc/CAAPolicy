...
def coarse_to_fine_coordinates(coarse_cor, ratio, topk=30000):
    ...
    fine_cor = fine_cor[None].repeat(ratio**2, 1, 1)  # [16, 2, N]

    device = fine_cor.device
    value = torch.meshgrid([torch.arange(ratio).to(device), torch.arange(ratio).to(device)])
    value = torch.stack(value, dim=2).reshape(-1, 2)

    fine_cor = fine_cor + value[:,:,None]
    if fine_cor.shape[-1] < topk:
        return fine_cor.permute(1,0,2).reshape(2,-1)
    else:
...
def project_points_on_img(points, rots, trans, intrins, post_rots, post_trans, bda_mat, pts_range,
                        W_img, H_img, W_occ, H_occ):
    with torch.no_grad():
        voxel_size = ((pts_range[2:] - pts_range[:2]) / torch.tensor([W_occ-1, H_occ-1])).to(points.device)
        points = points * voxel_size[None, None] + pts_range[:2][None, None].to(points.device)

        # project 2D point cloud (after bev-aug) onto multi-view images for corresponding 2D coordinates
        inv_bda = bda_mat.inverse()
        points = (inv_bda @ points.unsqueeze(-1)).squeeze(-1)
        
        # from lidar to camera
        points = points.view(-1, 1, 2)
        points = points - trans.view(1, -1, 2)

        inv_rots = rots.inverse()
        points = (inv_rots @ points.unsqueeze(-1))
        z_fake = torch.ones_like(points[:, :, :1, :])  # 假设 z=1，形状 [768, 4, 1, 1]
        points = torch.cat([points, z_fake], dim=2)  # 变成 [768, 4, 3, 1]
