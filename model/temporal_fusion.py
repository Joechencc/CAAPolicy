...
class TemporalFusion(nn.Module):  # Added nn.Module inheritance
    def __init__(self, cfg):
        ...
        self.T = 200 # Number of time steps to align
    ...
    def align_gt_bev_semantics(self, idx):
        ...
        """
        This funciton outputs the aligned BEV features for the current step idx in the batch.
        idx: current index in the batch from 0 to B-1
        aligned_bev_gt: the T-step's bev feature after affine grid sampling, shape: (T, 200, 200, 1)
        not_aligned_bev_gt: the T-step's bev feature before affine grid sampling, shape: (T, 200, 200, 1), for debugging purpose
        """
        ...
        # absolute poses: shape [num_valid+1, 3]
        valid_ego_pose = self.ego_pose_cache[
            idx + self.T - num_valid_past_frames : idx + self.T + 1
        ]
        # split history vs current
        hist_poses  = valid_ego_pose[:-1]  # [n,3]
        curr_pose   = valid_ego_pose[-1]   # (x0,y0,yaw0_deg)
        x0, y0, yaw0_deg = curr_pose.tolist()
        yaw0_rad = yaw0_deg * (math.pi/180.0)
        ...
        # offset for indexing the valid caches
        cache_offset = self.T - num_valid_past_frames
        # Align valid past frames
        # Iterate over the valid past frames in oldest-first order
        # import pdb; pdb.set_trace()
        for i, pose in enumerate(hist_poses):
            ...
            xi, yi, yawi_deg = pose.tolist()
            # delta in world coords
            dx = xi - x0
            dy = yi - y0
            # relative yaw
            yaw_rel = (yawi_deg * math.pi/180.0) - yaw0_rad

            # compute inverse affine for grid_sample
            c, s = math.cos(yaw_rel), math.sin(yaw_rel)
            # inverse translation (meters)
            tx_m = -(c*dx + s*dy)
            ty_m = -(-s*dx + c*dy)
            # normalize to [-1,1]
            tx = tx_m / half_w_m
            ...
            # TODO: Review the index calculation(for the BEV cache)
            bev_feature = self.gt_bev_cache[idx + cache_offset + i].unsqueeze(0).squeeze(2) # torch.Size([1, 1, 200, 200])

            # Construct the affine transformation matrix
            theta = torch.tensor([[ c,  s, tx],
                                [-s,  c, ty]], device=self.gt_bev_cache.device).unsqueeze(0)
            # Generate the sampling grid
            grid = F.affine_grid(theta, size=bev_feature.size(), align_corners=False)

            # Apply the affine transformation to the downsampled BEV feature
            aligned_bev_gt[cache_offset + i] = F.grid_sample(
            ...
            not_aligned_bev_gt[cache_offset + i] = bev_feature.squeeze(0).permute(1, 2, 0)
    ...
    def padded_align_bev(self, idx, pad_size=300, res=0.1):
        ...
        # 1) gather valid frames and their absolute ego poses
        num_valid = min(self.T-1, self.cur_idx2 - self.cur_start_idx2)
        # absolute poses: shape [num_valid+1, 3]
        valid_ego_pose = self.ego_pose_cache[
            idx + self.T - num_valid : idx + self.T + 1
        ]
        # split history vs current
        hist_poses  = valid_ego_pose[:-1]  # [n,3]
        curr_pose   = valid_ego_pose[-1]   # (x0,y0,yaw0_deg)
        ...
        # 2) prepare canvases
        aligned_bev = torch.zeros((self.T, pad_size, pad_size, 1), device=self.gt_bev_cache.device)
        global_bev  = torch.zeros((pad_size, pad_size, 1), device=self.gt_bev_cache.device)

        # half‐extent in meters of the pad canvas
        half_m = (pad_size * res) / 2.0
        # where to inset the 200×200 BEV
        inset = (pad_size - 200) // 2
        import pdb; pdb.set_trace()
        # 3) stamp each history BEV onto global canvas
        for i, pose in enumerate(hist_poses):
            xi, yi, yawi_deg = pose.tolist()
            # delta in world coords
            dx = xi - x0
            ...
            # pad small BEV into center of large canvas
            bev_small = self.gt_bev_cache[idx + self.T - num_valid + i]  # bev_small.shape: (1,200,200)
            bev_map   = bev_small.squeeze()  # (200,200)
            bev_pad   = torch.zeros((1, 1, pad_size, pad_size),
                                    device=bev_small.device)
            bev_pad[0, 0, inset:inset+200, inset:inset+200] = bev_map
            ...
            theta = torch.tensor([[[ c,  s, tx],
                                [-s,  c, ty]]], device=bev_pad.device)
            grid = F.affine_grid(theta, bev_pad.size(), align_corners=False)
            warped = F.grid_sample(bev_pad, grid, padding_mode='zeros', align_corners=False)

            # write into aligned_bev and update global
            out = warped.squeeze(0).permute(1,2,0)  # (pad,pad,1)
            aligned_bev[i] = out
            global_bev = torch.maximum(global_bev, out)

        # 4) stamp current frame (no transform)
        bev_curr = self.gt_bev_cache[self.T + idx]
        bev_pad_curr = torch.zeros((pad_size,pad_size,1), device=bev_curr.device)
        bev_pad_curr[inset:inset+200,inset:inset+200,0] = bev_curr.squeeze(-1)
        aligned_bev[-1] = bev_pad_curr
        global_bev = torch.maximum(global_bev, bev_pad_curr)

        # 5) visualize global composite
        os.makedirs('gt_bev_global', exist_ok=True)
        plt.figure(figsize=(8,8))
        plt.imshow(global_bev.cpu().numpy().squeeze(), origin='lower', cmap='viridis')
        plt.axis('off')
        plt.title('Global BEV Composite')
        plt.savefig('gt_bev_global/global_bev.png', bbox_inches='tight')
        plt.close()
