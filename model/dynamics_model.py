...
class DynamicsModel(nn.Module):  # Fixed typo in nn.module -> nn.Module
    ...
    def forward(self, data, dt=0.1):
        ...
        displacement_x_world_track = vehicle_velocity_x / 3.6 * dt + 0.5 * accel_x_world * dt**2
        displacement_y_world_track = vehicle_velocity_y / 3.6 * dt + 0.5 * accel_y_world * dt**2
        ...
        inputs = torch.cat((vehicle_velocity_x.reshape(-1,1), 
                            ...
                            reverse.reshape(-1,1), 
                            cos_yaw.reshape(-1,1), 
                            sin_yaw.reshape(-1,1)), dim=1).float()
