...
class WaypointPredict(nn.Module):
    ...
    def decoder(self, encoder_out, tgt_embedding, tgt_mask, tgt_padding_mask):
        ...
        pred_waypoints = pred_waypoints.transpose(0, 1)
    ...
    def predict(self, encoder_out, tgt):
        ...
        pred_waypoints = self.decoder(encoder_out, tgt_embedding, tgt_mask, tgt_padding_mask)
        pred_waypoints = self.output(pred_waypoints)[:, length - 1, :]

        pred_waypoints = torch.softmax(pred_waypoints, dim=-1)
        pred_waypoints = pred_waypoints.argmax(dim=-1).view(-1, 1)
        return pred_waypoints
