...
class PIDLateralController():
    ...
    def _pid_control(self, waypoint, vehicle_transform, direction=1):
        ...
        if direction == 1:  ### forward
            ...
            w_vec = np.array(
                [np.round(w_loc.x, 1) - np.round(ego_loc.x, 1), np.round(w_loc.y, 1) - np.round(ego_loc.y, 1), 0.0])
...
class Path_collector:
    ...
    def init_agent(self):
        self.pid_controller = VehiclePIDController(self.player, lateral_par, longitudinal_par, max_throttle=1.0,
    ...
    def tick(self, carla_world):
        if self.current_target[:2] != [self.net_eva._parking_goal.x,
                                       self.net_eva._parking_goal.y] or self.world._need_init_ego_state:  ### we also need to consider the initial position change for ego
        ...
        if self.step % self.process_frequency == 0:
            ...
            if self.solution_required:
                print('ego angle: ', self.player.get_transform().location.x, self.player.get_transform().location.y,
                      self.player.get_transform().rotation.yaw)  ## semantic  ## lidar pc
                ...
                if result is not None:
                    ...
                    for k in range(0, len(x), 1):
                        y_graph = x[
                                      k] - 16.0  # Rear center in ego coordinate x heading (since this path uses rear center)
                        x_graph = y[k] - 16.0
                        ...
                        y_world = math.cos(self.ab_yaw_init) * y_graph - math.sin(
                            self.ab_yaw_init) * x_graph + self.a_y_init
                        x_world = math.sin(self.ab_yaw_init) * y_graph + math.cos(
                            self.ab_yaw_init) * x_graph + self.b_x_init
                        yaw_world = np.deg2rad(
                            yaw_graph) - self.ab_yaw_init  ### notice here self.ab_yaw_init is negative
                    ...
                    self.positions.append([self.net_eva._parking_goal.x - 1.37 * math.cos(final_yaw_world),
                                           self.net_eva._parking_goal.y - 1.37 * math.sin(final_yaw_world),
                                           final_yaw_world, direction[k]])
                    ...
                    for ele1, ele2 in zip(ox_real, oy_real):
                        oy_real_w.append(math.cos(self.ab_yaw_init) * (ele1 - 16) - math.sin(self.ab_yaw_init) * (
                                    ele2 - 16) + self.a_y_init)
                        ox_real_w.append(math.sin(self.ab_yaw_init) * (ele1 - 16) + math.cos(self.ab_yaw_init) * (
                                    ele2 - 16) + self.b_x_init)
                    ...
                    plt.scatter(self.player.get_transform().location.y - 1.37 * math.sin(
                        np.deg2rad(self.player.get_transform().rotation.yaw)),
                                self.player.get_transform().location.x - 1.37 * math.cos(
                                    np.deg2rad(self.player.get_transform().rotation.yaw)), label='ego rear start')
                    ...
                    if self.player.get_transform().rotation.yaw > 0:  ## heading right
                        ...
                        diff_yaw = (-90 - self.player.get_transform().rotation.yaw + 180) % 360 - 180
                    self.player.apply_control(carla.VehicleControl(throttle=1, steer=diff_yaw * 0.1))
                return
            else:
                ...
                ego_rear_x = np.round(
                    ego_center_current_x - 1.37 * math.cos(np.deg2rad(self.player.get_transform().rotation.yaw)), 1)
                ego_rear_y = np.round(
                    ego_center_current_y - 1.37 * math.sin(np.deg2rad(self.player.get_transform().rotation.yaw)), 1)
                dist = np.round(math.hypot(ego_rear_x - self.mul_pos[self.path_stage][-1][0],
                                           ego_rear_y - self.mul_pos[self.path_stage][-1][1]), 2)
                ...
                if (dist >= self.hist_use and dist <= 0.5) or dist < 0.2 or self.finetuning:  # or dist < 0.1
                    ...
                    if feng_speed >= 1e-3 and not self.finetuning:  ### first make sure the vehicle stops when finishing to track one segment
                        ...
                        return
                    print('path segment completed and switch to the next path...', ' This is the final segment: ',
                          self.path_stage == len(self.mul_pos) - 1)

                    if self.path_stage == len(self.mul_pos) - 1:
                        print('We completed all paths')

                        reach_goal = abs(
                            self.player.get_transform().location.x - self.net_eva._parking_goal.x) < 0.5 and abs(
                            self.player.get_transform().location.y - self.net_eva._parking_goal.y) < 0.5
                        reach_goal = reach_goal and min(abs(self.player.get_transform().rotation.yaw),
                                                        180 - abs(self.player.get_transform().rotation.yaw)) < 0.5

                        print('Task done? ', reach_goal)
                        if reach_goal:  ###
                            print('\n\nIt takes {} seconds to finish one experiment'.format(
                                time.time() - self.start_time), ' simulation time: ', self.step / 30)
                            # #### trigger a new task
                            ...
                            rear_x_ego = self.player.get_transform().location.x - 1.37 * math.cos(
                                np.deg2rad(self.player.get_transform().rotation.yaw))  # np.round(,1)
                            rear_y_ego = self.player.get_transform().location.y - 1.37 * math.sin(
                                np.deg2rad(self.player.get_transform().rotation.yaw))  # np.round(,1)
                            ...
                            if abs(diff_yaw) > 0.4:  ### 5 degrees
                                print('finetuning by moving forward to adjust the heading angle')
                                self.player.apply_control(carla.VehicleControl(throttle=0.8, steer=diff_yaw * 0.1))
                                return
                            else:  ### backwards for
                                print('backwards...')
                        ...
                        self.pid_controller = VehiclePIDController(self.player, lateral_par, longitudinal_par,
                                                                   max_throttle=1.0, max_steering=max_steering)
                ...
                waypoint_index = self.r_trajectory.nearest_index(ego_rear_x,
                                                                 ego_rear_y)  # We add extra steps in the nearest_index function
                ...
                location = carla.Location(x=self.r_trajectory.cx[waypoint_index],
                                          y=self.r_trajectory.cy[waypoint_index], z=0)
                ...
                target_speed = 5.4  # 8.064 #4 #4.47 #2.24 ### similar to 5 miles/hour #4.2 #0.21 #3
    ...
    def hybrid_A_star_solution(self, semantic_img, voxels=None):
        if voxels is None:
            ...
            for i in range(N):  ## row
                for j in range(M):  ## column
                    if crop_img[i][j] == 81 or crop_img[i][j] == 153 or crop_img[i][j] == 128 or crop_img[i][
                        j] == 50:  ### Ground (81, 0, 81), Pole (153, 153, 153), Road (128, 64, 128), Lane (50, 234, 157)
                        continue
                        ...
                        for x_a, y_a in [[-cell_size / 2, -cell_size / 2], [cell_size / 2, cell_size / 2],
        ...
        self.ab_yaw_init = np.deg2rad(
            -self.player.get_transform().rotation.yaw)  ## clockwise is negative in ab coordinate
        ...
        gx = 16 + math.cos(self.ab_yaw_init) * (self.target_y - self.a_y_init) + math.sin(self.ab_yaw_init) * (
                    self.target_x - self.b_x_init)
        gy = 16 - math.sin(self.ab_yaw_init) * (self.target_y - self.a_y_init) + math.cos(self.ab_yaw_init) * (
                    self.target_x - self.b_x_init)
        ...
        if result is not None:
            #########################################
            if len(result[0]) <= 100:
                print('It takes {} seconds to find a path'.format((time.time() - start_time)), 'total waypoints: ',
                      len(result[0]))
    ...
    def get_model_data(self, data_frame):
        ...
        filtered_points = filtered_points[
            filtered_points[:, 2] > 0.2]  ###-height ##originall for lidar coordinate system
