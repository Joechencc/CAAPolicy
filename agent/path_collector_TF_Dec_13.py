...
class VehiclePIDController():
    ...
    def run_step(self, target_speed, waypoint, direction = 1): ### add a direction to enable reverse tracking
        ...
        steering_multiplier = 1.0 - 0.3 * steer_angle  # Reduce speed up to 30% when turning
...
class PIDLongitudinalController():
    ...
    def _pid_control(self, target_speed, current_speed):
        ...
        Estimate the throttle/brake of the vehicle based on the PID equations
            ...
            :return: commands of throttle
        """
        # Defines a buffer zone to prevent abrupt braking when slightly over the target speed.
        brake_buffer = 1.0  # km/h

        # Condition 1: If current speed is well above the target, brake.
        if current_speed > target_speed + brake_buffer:
            error = target_speed - current_speed
        # Condition 2: If current speed is in the buffer zone (slightly over target), coast.
        elif current_speed > target_speed:
            error = 0.0 # Set error to zero to produce zero acceleration (no throttle, no brake).
        # Condition 3: If current speed is below target, accelerate.
        else:
            error = target_speed - current_speed
    ...
    def _pid_control(self, waypoint, vehicle_transform, direction = 1):
        ...
        if direction == 1: ### forward
            ...
            if w_vec_norm < 1e-4:
                ...
                _dot = math.acos(np.clip(np.dot(w_vec, v_vec) / (w_vec_norm * np.linalg.norm(v_vec)), -1.0, 1.0))
        ...
        if self.step % self.process_frequency == 0:
            ...
            if self.solution_required:   
                ...
                if result is not None:
                    ...
                    self.soft_start_thre = max(self.soft_start_thre, 0.2)
                return    
            else:
