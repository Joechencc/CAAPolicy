...
class ParkingAgent:
    def __init__(self, network_evaluator: NetworkEvaluator, args, folder_name):
    ...
    def init_agent(self):
        ...
        self.dynamic_traj = []
        self.task_num += 1
        subfolder = os.path.join(self.output_path, f"task{self.task_num}")
        subfolder_m = os.path.join(self.output_path, f"task{self.task_num}", "measurements")
        os.makedirs(subfolder, exist_ok=True)
        os.makedirs(subfolder_m, exist_ok=True)
    ...
    def tick(self, folder_name):
        ...
        if self.step % self.process_frequency == 0:
            ...
            measurements_file = os.path.join(folder_name,f"task{self.task_num}" , 'measurements' , f"{(self.step - 30) // 3:04d}.json")
