...
class ParkingAgent:
    ...
    def init_agent(self):
        ...
        rear_to_ego = torch.from_numpy(veh2cam_dict['rgb_back']).float().unsqueeze(0)
        rear_left_to_ego = torch.from_numpy(veh2cam_dict['rgb_back_left']).float().unsqueeze(0)
        rear_right_to_ego = torch.from_numpy(veh2cam_dict['rgb_back_right']).float().unsqueeze(0)
        self.extrinsic = torch.cat([front_to_ego, front_left_to_ego, front_right_to_ego, front_right_to_ego, rear_left_to_ego, rear_right_to_ego], dim=0)
    ...
    def get_model_data(self, data_frame):
        ...
        back_left_final, self.rgb_back_left = self.image_process(data_frame['rgb_back_left'])
